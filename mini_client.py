...
game_global = dotwar_classes.Game("TESTGAME", "C:\\Users\\1zada\\PycharmProjects\\dotwar")
game_global.load()
# print(game.system)
...
def summarize(vessel_name, last_seen, game):
	...
	for event in events:
		...
		if event["type"] == "capture":
			...
			desc = ["vessel", event["args"][0], "started burn", str(event["args"][1]), "while at coords",
					str(event["args"][2])]
		print(str(event["time"]), "\t", " ".join(desc))
	print("Summary complete.")


scan(vessel_name_global, game_global)
summarize(vessel_name_global, datetime.datetime.fromisoformat(game_global.system["game"]["created_on"]), game_global)
